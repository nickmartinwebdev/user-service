...
use std::env;
use user_service::{CreateUserRequest, DatabaseConfig, UserService};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://user_service:password@localhost:5432/user_service".to_string()
    });
    ...
    let is_valid = user_service
        .verify_password(user.id, "SecurePassword123!")
        .await?;
    println!(
    ...
    println!(
        "Found user: {} (ID: {})",
        user_by_email.name, user_by_email.id
    );
